...
// ============================================================
// Agent Chat — Tool Use (Anthropic-compatible)
// ============================================================
//
// These types back the subagent system. The endpoint is the same as
// `ChatRequest` (`POST /anthropic/v1/messages`), but the agent loop
// needs to (a) send `tools` definitions, (b) send multi-block content
// for `tool_use` and `tool_result`, and (c) parse `tool_use` blocks
// from responses. Existing `ChatRequest`/`ChatMessage` stay untouched.

/// Tool definition sent in the `tools` field of an agent chat request.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ToolSpec {
    pub name: String,
...
/// Message content for the agent system.
///
/// Anthropic accepts either a plain string (for simple user prompts) or
/// an array of content blocks (for `tool_use` and `tool_result`).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// Single content block inside an agent message.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum AgentContentBlock {
    /// Plain text content.
    Text { text: String },
    /// Assistant's request to call a tool.
    ToolUse {
        id: String,
        name: String,
...
/// Message in the agent loop. `role` is `user` or `assistant`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Chat request for the agent system.
///
/// Same endpoint as `ChatRequest` but supports the `tools` field and
...
/// Content block in an LLM response.
#[derive(Debug, Clone, Deserialize)]
pub struct AgentResponseBlock {
    #[serde(rename = "type")]
    pub block_type: String,
    #[serde(default)]
    pub text: Option<String>,
...
/// Response from an agent chat call.
#[derive(Debug, Clone, Deserialize)]
pub struct AgentChatResponse {
    ...
    pub role: String,
    #[serde(default)]
    pub content: Vec<AgentResponseBlock>,
    #[serde(default)]
    pub stop_reason: Option<String>,
    #[serde(default)]
    pub usage: Option<ChatUsage>,
